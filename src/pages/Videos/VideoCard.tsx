  ...
  return (
    <article className="card video-card">
      <div className="video-card__media">
        {item.coverImageUrl ? (
          <img src={item.coverImageUrl} alt={item.title ?? item.name ?? 'Video cover'} loading="lazy" />
        ) : (
          <div className="video-card__placeholder">Video</div>
        )}
      </div>
      <div className="video-card__body">
        <h3 className="video-card__title">{item.title ?? item.name ?? 'Untitled video'}</h3>
